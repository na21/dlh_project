    ...
    select
      ...
      , case
          ...
          and i.outtime_lag > (DATETIME_SUB(i.intime, INTERVAL '24 HOUR'))
            then DATETIME_SUB(i.intime, '1 HOUR'::interval * cast(round((DATETIME_DIFF(i.intime, i.outtime_lag, 'hour')/2)) as bigint))
      ...
      , case
          ...
          and i.intime_lead < (DATETIME_ADD(i.outtime, INTERVAL '24 HOUR'))
            then DATETIME_ADD(i.outtime, '1 MINUTE'::interval * cast(round((DATETIME_DIFF(i.intime_lead, i.outtime, 'minute')/2)) as bigint))
